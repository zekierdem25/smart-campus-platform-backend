...
public class EventsController : ControllerBase
{
    ...
    public async Task<ActionResult<object>> RegisterForEvent(Guid id, [FromBody] EventRegistrationDto? dto)
    {
        ...
        // Check wallet balance for paid events before starting transaction to fail early
        if (evt.IsPaid && evt.Price.HasValue && evt.Price > 0)
        {
            var wallet = await _context.Wallets.FirstOrDefaultAsync(w => w.UserId == userId);
            if (wallet == null || wallet.Balance < evt.Price.Value)
        ...
        try
        {
            ...
            {
                var wallet = await _context.Wallets.FirstOrDefaultAsync(w => w.UserId == userId);
                // Balance check already done, but good to be safe or just proceed since we are in transaction
                if (wallet != null)
                {
                    wallet.Balance -= evt.Price.Value;
                    ...
                    var walletTransaction = new Transaction
                    {
                        ...
                        Amount = evt.Price.Value,
                        BalanceAfter = wallet.Balance,
                        ReferenceType = "EventRegistration",
                        ReferenceId = registration.Id, // Note: ID is generated by EF/client-side-generation? EventRegistration.Id is GUID but likely not set until add?
                                                       // Wait, Model `EventRegistration` doesn't have default ID in code shown?
                                                       // Let's check model... usually it has [Key] public Guid Id { get; set; } = Guid.NewGuid();
                        Description = $"Etkinlik kaydı: {evt.Title}"
                    };
                    // Manually ensure ID is available if not auto-generated before save
                    if (registration.Id == Guid.Empty) registration.Id = Guid.NewGuid();
                    walletTransaction.ReferenceId = registration.Id;
    ...
    public async Task<IActionResult> CancelRegistration(Guid eventId, Guid regId)
    {
        ...
        try
        {
            ...
            // Refund if paid
            if (registration.Event.IsPaid && registration.Event.Price.HasValue && registration.Event.Price > 0)
            {
                var wallet = await _context.Wallets.FirstOrDefaultAsync(w => w.UserId == userId);
                if (wallet != null)
                {
                    wallet.Balance += registration.Event.Price.Value;
                    wallet.UpdatedAt = DateTime.UtcNow;
                    _context.Update(wallet);
                    ...
                    var walletTransaction = new Transaction
                    {
                        ...
                        Amount = registration.Event.Price.Value,
                        BalanceAfter = wallet.Balance,
                        ReferenceType = "EventRefund",
                        ReferenceId = registration.Id,
                        Description = $"Etkinlik iptal iadesi: {registration.Event.Title}"
                    };
                    _context.Transactions.Add(walletTransaction);
                }
            }
